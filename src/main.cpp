	 ...
	 double ref_vel = 49.5; //mph 
          ...
          for(int i=0; i<sensor_fusion.size();i++)
	  {
         ...
         // later we will interpolate these waypoint with a spline and fill it in with more points that control speed
          ...
          // either we will reference the starting point as where the car is or at previous paths end points
          double ref_x = car_x;
          double ref_y = car_y;
          double ref_yaw = deg2rad(car_yaw);
          // if the previous size is almost empty, use the car as starting point
          if(prev_size < 2)
          {
	  ...
	  //Define the local actual (x, y) points we wil use for the planer
